...
It removes the value from the stack, multiplies the removed stack value with the register value, then stores the result back in the register. 
...
function minilang(prg) {
  prg.split(' ').forEach(cmd => {
    switch (cmd) {
      case 'PUSH':
        push(register, stack);
        break;
...
function push(register1, stack) {
  stack.push(register1);
}

function add(register1, stack) {
  return register = register1 + stack.pop();
}

function sub(register1, stack) {
  return register = register1 - stack.pop();
}

function mult(register1, stack) {
  register = register1 * stack.pop();
}
function div(register1, stack) {
  return register = parseInt(register1 / stack.pop());
}

function remainder(register1, stack) {
  return register = parseInt(register1 % stack.pop());
}

function pop(register1, stack) {
